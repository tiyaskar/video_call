...
function App() {
  ...
  useEffect(() => {
    ...
    userRef.child('users').child("23456").on('value', (snapshot) => {
      // console.log(snapshot.incoming)
      // let newUserState = [];
      // var x= snapshot.child('name').val();
      // console.table(snapshot.val())
      if (snapshot.exists()) {
        console.log("23456 exists")
        setCheck(true)
        userRef.child('users').child("23456").set(
          {
            connId: "23456"
          }
        )
      } else {
        console.log("23456 doesnot exists")
        // window.location.reload(false);
  ...
  const endCall = () => {
    ...
    //   currentCall = "";
    const userRef = firebase.database().ref();
    userRef.child('users').child("23456").remove();
    firebase.database().ref().child('users').child("23456").on('value', (snapshot) => {
    //   // console.log(snapshot.incoming)
    //   // let newUserState = [];
    //   // var x= snapshot.child('name').val();
    //   // console.table(snapshot.val())
  ...
  return (
    <div className="container-fluid">
      ...
      <div className="main__controls">
        ...
        <div className="main__controls__block" onClick={endCall}>
          <div className="main__controls__button">
